...
os.makedirs(os.path.abspath(pathStore), exist_ok=True)
    ...
    for i in range(len(dataMyFile)):
        ...
        try:
            ...
            if addFileGuard:
                ...
                os.makedirs(os.path.abspath(pathStore, WP_ID), exist_ok=True)

                # Dodanie pliku we wskazane miejsce
                dataFile.save(os.path.abspath(pathStore, WP_ID, filename))
        ...
        for i in range(len(dataMyFile)):
            ...
            for i in range(len(dataPathSplit)-1):
                ...
                if addFileArrayGuard:
                    UID = str(uuid.uuid4())
                    os.makedirs(os.path.abspath(pathStore, awp_id, UID), exist_ok=True)
            ...
            if addFileGuard:
                dataFile.save(os.path.abspath(pathStore, awp_id, dataPathSplit[-1]))
                UID = str(uuid.uuid4())
...
def make_folder(current_user):
    ...
    try:
        #Tworzenie folderu jeżeli nie istnieje
        os.makedirs(os.path.abspath(pathStore, WP_ID, UID), exist_ok=True)
        new_file = Files(public_id=PID,
                         uid=UID,
                         wai=WAI,
                         # binaryCode=None,
...
def rename_file(current_user):
    ...
    if newname != '':
        if files.isFolder:
            ...
            os.rename(os.path.abspath(pathStore, PID+WAI, oldFileName), os.path.abspath(pathStore, PID+WAI, newR))
    ...
    if myfiles.isFolder:
        for root, directories, files in os.walk(dirName):
            for filename in files:
                ...
                filePath = os.path.abspath(root, filename)
                t = (mywai+myfiles.uid+'/')
        ...
        for u in uids:
            ...
            if len(uids) == 1:
                for file in files:
                    if file.uid == uid[0]:
                        if file.isFolder:
                            path = os.path.abspath(pathStore, file.public_id + jsonWAI(mywai), file.uid + '/')
                            zip_file = zipfile.ZipFile((os.path.abspath(pathStore, file.public_id, 'Store.zip')), 'w', compression=zipfile.ZIP_DEFLATED)
                            retrieve_file_paths(path, file, zip_file, file, jsonWAI(mywai))
                            zip_file.close()

                            with open(os.path.abspath(pathStore, file.public_id, 'Store.zip'),'rb') as z:
                            ...
                            try:
                                with open(os.path.abspath(pathStore, file.public_id + file.wai, file.name),'rb') as f:
                            ...
                            try:
                                ...
                                with open(os.path.abspath(pathStore, file.public_id + file.wai, fn), 'rb') as f:
                                    encode_string = base64.encodebytes(f.read())
                ...
                try:
                    os.remove(os.path.abspath(pathStore, current_user.public_id, 'Store.zip'))
                except:
                    pass

                zip_file = zipfile.ZipFile((os.path.abspath(pathStore, current_user.public_id, 'Store.zip')), 'a',
                                           compression=zipfile.ZIP_DEFLATED)

                for l in listtozip:
                    path = os.path.abspath(pathStore, current_user.public_id + l.wai, l.uid)
                    retrieve_file_paths(path, l, zip_file, current_user, l.wai)

                zip_file.close()

                with open(os.path.abspath(pathStore, current_user.public_id, 'Store.zip'), 'rb') as z:
                    encode_string = base64.encodebytes(z.read())
                    res = Response(encode_string, headers={'name': 'Files', 'typ': 'application/zip'})
                    return res
    ...
    if header == 'sharedDownload':
        if len(uids) == 1:
            try:
                ...
                uid.append(fil.uid)
                if fil.isFolder:
                    path = os.path.abspath(pathStore, fil.public_id + fil.wai, fil.uid + '/')
                    zip_file = zipfile.ZipFile((os.path.abspath(pathStore, fil.public_id, 'Store.zip')), 'w',
                                               compression=zipfile.ZIP_DEFLATED)
                    retrieve_file_paths(path, fil, zip_file, fil, fil.wai)
                    zip_file.close()

                    with open(os.path.abspath(pathStore, fil.public_id, 'Store.zip'), 'rb') as z:
                        encode_string = base64.encodebytes(z.read())
                        res = Response(encode_string, headers={'name': 'Files', 'typ': 'application/zip'})
                        return res

                with open(os.path.abspath(pathStore, fil.public_id + fil.wai, fil.name), 'rb') as f:
                    encode_string = base64.encodebytes(f.read())
            ...
            if len(wai) == 1:
                ...
                zip_file = zipfile.ZipFile((os.path.abspath(pathStore, current_user.public_id, 'Store.zip')), 'a',
                                           compression=zipfile.ZIP_DEFLATED)
                for l in listtozip:
                    path = os.path.abspath(pathStore, l.public_id + l.wai, l.uid)
    ...
    try:
        ...
        try:
            for file in filesToDelete:
                path = os.path.abspath(pathStore,WP_ID,file.name)
                if file.isFolder:
                    shutil.rmtree(path)
                else:
                    os.remove(path)
...
def get_file_show(current_user):
    ...
    with open(os.path.abspath(pathStore, files.public_id + files.wai, files.name), 'rb') as bites:
...
def add_version(current_user):
    ...
    path = os.path.abspath(pathStore, current_user.public_id)
    ...
    print(file.name)
    print(file.message)
    path_dest_main_file = os.path.abspath(pathStore, current_user.public_id + file.wai, file.name)
    ...
    if len(ver) == 0:
        ...
        afile.stream.seek(0)
        afile.save(os.path.abspath(pathStore, current_user.public_id + awai, file.name))
...
def get_version_and_show(current_user):
    ...
    try:
        ...
        with open(os.path.abspath(pathStore, current_user.public_id+jsonWAI(mywai), file.name), 'rb') as bites:
            encode_string = base64.encodebytes(bites.read())
...
def download_version(current_user):
    ...
    try:
        ...
        with open(os.path.abspath(pathStore, current_user.public_id + jsonWAI(mywai), file.name), 'rb') as f:
            encode_string = base64.encodebytes(f.read())
            res = Response(encode_string, headers={'name': file.name, 'typ': file.fileExtension})
            repo.git.checkout(file.commit_id)
            return res
    ...
    for user in users:
        # if user.public_id != current_user.public_id:
            ...
            if user.avatar != None:
                ...
                with open(os.path.abspath('avatardefault'), 'rb') as z:
                    encode_string = base64.encodebytes(z.read())
                    my_j = encode_string.decode('utf-8')
                    my_j = 'data:image/png;base64,' + my_j
                    file_data['select'] = my_j
...
def get_file_to_show(current_user):
    ...
    try:
        with open(os.path.abspath(pathStore, fileInfo.public_id+jsonWAI(mywai), fileInfo.name), 'rb') as bites:
            encode_string = base64.encodebytes(bites.read())
            my_j = encode_string.decode('utf-8')
            return my_j
    except:
    ...
    try:
        with open(os.path.abspath(pathStore, fileInfo.public_id+jsonWAI(mywai), fn), 'rb') as bites:
            encode_string = base64.encodebytes(bites.read())
            my_j = encode_string.decode('utf-8')
            return my_j
    except:
...
def get_shared_show(current_user):
    ...
    with open(os.path.abspath(pathStore, fileInfo.public_id+fileInfo.wai, fileInfo.name), 'rb') as bites:
        encode_string = base64.encodebytes(bites.read())
        my_j = encode_string.decode('utf-8')
        return my_j
        ...
        for user in users:
            ...
            if u.admin:
                ...
                try:
                    ...
                    path = os.path.abspath(pathStore, user)
                    shutil.rmtree(path)
                except:
                    pass
...
def get_images(current_user):
    ...
    os.makedirs(os.path.abspath(pathStore, current_user.public_id, 'imagesresize'), exist_ok=True)
    for myfile in myfiles:
        im1 = Image.open((os.path.abspath(pathStore, current_user.public_id+mywai, myfile.name)))
        ...
        try:
            ...
            pass
        im3 = im2.convert('RGB')
        im3.save(os.path.abspath(pathStore, current_user.public_id, 'imagesresize', name))

        with open(os.path.abspath(pathStore, current_user.public_id, 'imagesresize', name), 'rb') as bites:
            encode_string = base64.encodebytes(bites.read())
            my_j = encode_string.decode('utf-8')
    ...
    shutil.rmtree(os.path.abspath(pathStore, current_user.public_id, 'imagesresize'))
...
def add_avatar(current_user):
    ...
    try:
        os.makedirs(os.path.abspath(pathStore, current_user.public_id, 'avatar'), exist_ok=True)
        data.save(os.path.abspath(pathStore, current_user.public_id, 'avatar', 'avatar.jpg'))
        im1 = Image.open((os.path.abspath(pathStore, current_user.public_id, 'avatar', 'avatar.jpg')))
        im2 = im1.resize((width, height), Image.NEAREST)
        im3 = im2.convert('RGB')
        im3.save(os.path.abspath(pathStore, current_user.public_id, 'avatar', 'ravatar.jpg'))

        with open(os.path.abspath(pathStore, current_user.public_id, 'avatar', 'ravatar.jpg'), 'rb') as z:
            encode_string = base64.encodebytes(z.read())
            u = User.query.filter_by(public_id=current_user.public_id).first()
...
def login():
    ...
    if check_password_hash(user.password, data['password']):
        ...
        path = os.path.abspath(pathStore, user.public_id)
        try:
            repo = git.Repo(path)
        except:
            git.Repo.init(path)
...
def add_user(current_user):
    ...
    if current_user.admin:
        ...
        path = os.path.abspath(pathStore, p_id)
        git.Repo.init(path)
        try:
            repo = git.Repo(path)
            repo.config_writer().set_value("user", "name", "myusername").release()
